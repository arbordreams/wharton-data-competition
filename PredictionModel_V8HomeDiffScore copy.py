...
def categorize_situation(row):
    ...
    if 'empty_net' in home_line or 'empty_net' in away_line:
        return 'EN'
    elif 'PP' in home_line or 'PP' in away_line:
        return 'ST'
    else:
        return 'EV'
...
def get_team_stats(games_df):
    ...
    metrics = ['xg', 'shots', 'assists', 'max_xg', 'toi']
    ...
    if 'home_penalties_committed_ST' in games_df.columns: home_cols.append('home_penalties_committed_ST'); home_rename[
        'home_penalties_committed_ST'] = 'penalties_committed_st'
    if 'away_penalties_committed_ST' in games_df.columns: home_cols.append('away_penalties_committed_ST'); home_rename[
        'away_penalties_committed_ST'] = 'penalties_drawn_st'
    ...
    if 'away_penalties_committed_ST' in games_df.columns: away_cols.append('away_penalties_committed_ST'); away_rename[
        'away_penalties_committed_ST'] = 'penalties_committed_st'
    if 'home_penalties_committed_ST' in games_df.columns: away_cols.append('home_penalties_committed_ST'); away_rename[
        'home_penalties_committed_ST'] = 'penalties_drawn_st'
...
# ---------------------------------------------------------
# 5. MODEL PREPARATION (Ensemble & Feature Expansion)
# ---------------------------------------------------------
print("\n--- Preparing Data with Extended Features (Diff + Home) ---")
...
def add_features(games_df, profiles, ratings_dict):
    ...
    for col in profiles.columns:
        ...
        # Home Feature (Doubling the features as requested)
        feature_list.append(f'home_{col}')

    return merged, feature_list


# Prepare Train/Test with Expanded Features
train_prepared, feature_cols = add_features(train_games, team_profiles_train, global_ratings_train)
test_prepared, _ = add_features(test_games, team_profiles_train, global_ratings_train)

# --- ROBUST FILLNA LOGIC ---
# 1. Fill input columns (home_rating_*, away_rating_*) with Means first
...
# 2. Fill any remaining diff NaNs with 0 (neutral)
# This handles edge cases where means might still be NaN or calculation artifacts
...
xgb_model = xgb.XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.05, eval_metric='logloss')

hgb = HistGradientBoostingClassifier(learning_rate=0.05, max_depth=3, max_iter=450, min_samples_leaf=20,
                                     random_state=42)

rf = RandomForestClassifier(n_estimators=350, max_depth=8, min_samples_leaf=10, n_jobs=-1, random_state=42)

lr = make_pipeline(StandardScaler(), LogisticRegression(C=1.0, random_state=42))

print(f"Training Ensemble on {len(feature_cols)} features...")
ensemble = VotingClassifier(
    estimators=[('xgb', xgb_model), ('hgb', hgb), ('rf', rf), ('lr', lr)],
    #estimators=[('xgb', xgb_model), ('hgb', hgb), ('rf', rf), ('lr', lr)],
    voting='soft'
)
...
# 2. Retrain Ensemble
full_prepared, _ = add_features(game_data, team_profiles_full, final_global_ratings)
full_prepared = full_prepared.fillna(fill_values).fillna(0)
ensemble.fit(full_prepared[feature_cols], full_prepared['target_home_win'])

# 3. Power Score
# For Power Scores, we compare Team vs Average.
# Diff = Team - Avg. Home Stat = Team.
...
for team in team_profiles_full.index.unique():
    row = {'team': team}
    # Global
    team_rating = final_global_ratings.get(team, 1500)
    row['diff_global_rating'] = team_rating - avg_global_rating
    row['home_global_rating'] = team_rating

    # Stats
    for col in team_profiles_full.columns:
        val = team_profiles_full.loc[team, col]
        row[f'diff_{col}'] = val - avg_profile[col]
        row[f'home_{col}'] = val
...
ranking = power_score_df[['team', 'Model_Win_Prob', 'diff_global_rating']].sort_values('Model_Win_Prob',
                                                                                       ascending=False)
ranking.to_csv('rst_ensemble_power_rankings.csv', index=False)
print("Power Rankings saved.")

# 4. Predict New Matches
new_matches_prepared, _ = add_features(new_matches, team_profiles_full, final_global_ratings)
new_matches_prepared = new_matches_prepared.fillna(fill_values).fillna(0)
