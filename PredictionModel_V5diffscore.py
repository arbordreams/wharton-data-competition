...
# ---------------------------------------------------------
# 4. TEAM STATS (Process Metrics + Goalie)
# ---------------------------------------------------------
...
def add_diff_features(games_df, profiles, ratings_dict):
    ...
    # This captures the RELATIVE advantage directly
    feature_list = []
...
# For difference columns, if input was missing, diff is likely NaN. Fill with 0 (neutral).
...
ranking.to_csv('rst_V5diffscore_team_power_rankings_diff.csv', index=False)
...
new_matches.to_csv('rst_V5diffscore_round_1_predictions_diff.csv', index=False)
...
perf_df = pd.DataFrame({
    ...
    'Value': [train_acc, test_acc, train_logloss, test_logloss]
})
